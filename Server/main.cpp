...
private slots:
    ...
    void onMessageReceived(const NetworkMessage &message, QTcpSocket *client) {
        ...
        case WALLET_QUERY_REQUEST: {  // 400
            ...
            // 添加详细调试信息
            qDebug() << "=== 处理钱包查询请求 ===";
            qDebug() << "原始消息数据:" << QJsonDocument(message.data).toJson();
            qDebug() << "解析到的用户名:" << username;
            qDebug() << "用户名是否为空:" << username.isEmpty();
            qDebug() << "用户名长度:" << username.length();
        ...
        case RECHARGE_RECORDS_REQUEST: {  // 应该是 24
            QString username = message.data["username"].toString();
            qDebug() << "处理充值记录查询请求，用户名:" << username;

            NetworkMessage reply;
            reply.type = RECHARGE_RECORDS_RESPONSE;  // 应该是 25

            QSqlQuery query;
            query.prepare("SELECT r.amount, r.old_balance, r.new_balance, r.recharge_time, "
                          "u.username "
            ...
            if (query.exec()) {
                ...
                while (query.next()) {
                    ...
                    recordObj["amount"] = query.value("amount").toDouble();
                    recordObj["before_balance"] = query.value("old_balance").toDouble();
                    recordObj["after_balance"] = query.value("new_balance").toDouble();
                    recordObj["recharge_time"] = query.value("recharge_time").toDateTime().toString("yyyy-MM-dd HH:mm:ss");
                    recordObj["username"] = query.value("username").toString();
                ...
                reply.data["count"] = count;
                qDebug() << "充值记录查询成功，找到" << count << "条记录";
            } else {
                reply.data["success"] = false;
                reply.data["message"] = "查询充值记录失败: " + query.lastError().text();
                qDebug() << "充值记录查询失败:" << query.lastError().text();
            }
